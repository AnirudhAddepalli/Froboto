...
public class EpsonRc7DriverConfig
{
    ...
    public EpsonRc7DriverConfig(UserOptions userOptions)
    {
        try
        {
        ...
        {
            MessageBox.Show("Fatal Error: Robot requires json configuration file with valid Locations.", "Application Fatal Error");
    ...
    public RobotLocation GetRobotPositionInfo(string pointName)
    {
        ...
        if (!found)
        {
            MessageBox.Show("Failed to find Location: " + robotLocation, "Application Fatal Error");
            Application.Current.Shutdown(1);
        }
