...
namespace SprayingSystem.Models
{
    ...
    public class SprayingSystemConfig
    {
        ...
        public static SprayingSystemConfig LoadSettings()
        {
            return LoadSettings(FileName);
        }

        public static SprayingSystemConfig LoadSettings(string filename)
        {
            ...
            catch(Exception e)
            {
                if (!e.Message.StartsWith("Could not find file"))
                {
                    MessageBox.Show(
                        e.Message +
                        "\n\nNOTE: Be careful when modifying data in the config file. Keep an old copy that works to compare where you have an error.",
                        "Configuration File Error (JSON File)");
                }
